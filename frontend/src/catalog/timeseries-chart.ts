...
// for large datasets. Plotly is loaded lazily on first render via a dynamic
// import so the large bundle is only fetched when a chart is actually rendered.
...
/** Maps standard Plotly trace types to their WebGL equivalents for performance. */
const WEBGL_TYPE_MAP: Record<string, string> = {
  scatter: 'scattergl',
  bar: 'bar',
};

function toWebGLTraces(
  traces: Array<Record<string, unknown>>,
): Array<Record<string, unknown>> {
  return traces.map(trace => {
    const type = typeof trace['type'] === 'string' ? trace['type'] : 'scatter';
    const webglType = WEBGL_TYPE_MAP[type] ?? type;
    return {
      mode: 'lines+markers',
      ...trace,
      type: webglType,
...
export class TimeseriesChart extends LitElement {
  ...
  @property({ type: Object }) layout: Record<string, unknown> = { xaxis: { type: 'date' } };
  ...
  private async renderChart(): Promise<void> {
    ...
    try {
      ...
      const webglTraces = toWebGLTraces(this.traces);
      ...
      if (!this.plotlyLoaded) {
        ...
        await Plotly.react(this.chartEl.value, webglTraces, mergedLayout, config);
      }
    } catch (err) {
...
declare global {
  interface HTMLElementTagNameMap {
    'ci-timeseries-chart': TimeseriesChart;
  }
}
