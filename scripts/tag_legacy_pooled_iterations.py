...
confirmed MERGEd across >=2 distinct OrchestratorRuns before the I5 fix.
...
Background (I5 / I3, see CHANGELOG.md)
---------------------------------------
Before the I5 fix, ``Iteration.node_id`` was the bare shape
...
(last-write-wins). After I5, new Iteration node_ids are run-scoped:
...
A **bare-id node_id does NOT mean the node is corrupt.** A council review +
...
* Any run-scoped node (``node_id`` contains ``'::orch_run::'``) -- these are
  post-I5 and structurally cannot collide across runs.
...
historical nodes; it does not race the I5 write-path fix (which governs only
newly-created Iteration nodes) and carries no risk of mixed-type or
mixed-shape state. It may be run at any time, before or after I5 is deployed
to a given server, and re-run as often as desired (see Idempotency below).
Running it is a maintenance convenience, not a precondition for shipping I5.
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "across >=2 distinct OrchestratorRuns before the I5 fix. Never "
