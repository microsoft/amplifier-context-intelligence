...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    try:
        ...
        # SchemaMeta baseline singleton (section 10.2 of the cursor-durability
        # spec). Deliberately called ONLY here -- startup, single-writer -- NOT
        # from ensure_neo4j_schema (which also runs on every Neo4jGraphStore's
        # first flush and from doctor --fix; see ensure_schema_version_baseline's
        ...
        # finite ceiling IS set, this drains the deferred tail over time instead
        # of stranding it until a restart or a new event (see
        # _crash_recovery_sweep_loop and config.crash_recovery_sweep_interval_seconds).
    ...
    try:
        ...
        # Cancel the periodic crash-recovery sweep (if one was started) before
        # closing drivers. It is created INSIDE the B1 boundary, so a startup
        # failure may leave it as None -- guard for that. CancelledError is the
        # expected result of cancel() and is suppressed.
