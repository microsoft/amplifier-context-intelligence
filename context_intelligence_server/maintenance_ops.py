...
(WS-3c) and, later, the standalone out-of-band migration script (spec
sec 5.4, not built in this change).

This module writes NO new dedup/repair algorithm (D-F, WS-3 spec sec 0/5.1):
...
- a bounded pre-op quiesce sleep (spec sec 5.3) to let ordinary in-flight
...
directly if it ever wants coordinator-aware bookkeeping. Per D-D/5.2, no
...
async def run_maintenance_operation(
    ...
    *,
    ...
    with ``n = duplicates_removed + nodes_tagged`` (spec sec 6.1). On any
