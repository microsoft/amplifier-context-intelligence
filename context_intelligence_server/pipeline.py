...
async def process_event(
    ...
    try:
        ...
        if session_id and timestamp and worker.services.blob_store:
            ...
            # second write silently overwrites the first via os.replace, while
            # the first Event node's $blob_ref still points at that URI (now
            # holding the wrong payload). tool_call_id is present on ALL event
            # types that carry it, not just tool:* (see default.py's own
            # comment on this), so it is safe to read unconditionally here.
