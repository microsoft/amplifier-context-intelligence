"""Tests for the MaintenanceCoordinator seam.

Covers:
probe tri-state + fail-open, TTL caching, single-flight, the latch-defect
regression, the op-running/constraint-present interaction, and transition
logging. Allow-list/gate/status HTTP-surface tests live in test_main.py;
drain-loop offset-ordering tests live in test_registry.py; the docs
tripwire lives in test_docs_entrypoint.py; the maintenance-mode admin
surface also has coverage in test_main.py.
"""
...
# ---------------------------------------------------------------------------
# Tri-state probe + fail-open rule
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Single-flight: N concurrent callers at cache expiry -> ONE probe
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# THE LATCH DEFECT, DIRECTLY: probe flips False->True with NO restart
# ---------------------------------------------------------------------------
...
class TestNoRestartSelfClear:
    ...
    async def test_non_vacuous_without_ttl_expiry_gate_stays_stale(self) -> None:
        """Non-vacuity proof: if we DON'T wait out the TTL,
...
# ---------------------------------------------------------------------------
# op_running holds the gate closed even when constraint IS present
# ---------------------------------------------------------------------------
...
class TestOpRunningKeepsGateClosed:
    async def test_op_running_with_constraint_present_still_closed(self) -> None:
        ...
        assert await coord.gate_closed() is True  # op_running term
...
# ---------------------------------------------------------------------------
# Transition logging fires EXACTLY once per transition
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# No driver bound => gate OPEN (the existing-suite no-regression rule)
# ---------------------------------------------------------------------------
