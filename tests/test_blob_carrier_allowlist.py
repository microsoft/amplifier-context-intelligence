"""Tests for the blob-carrier allowlist runtime tripwire.
...
def test_assert_carrier_registered_raises_for_unregistered_carrier() -> None:
    ...
    Proves the tripwire is non-vacuous: it actually fires. Simulates a
    plausible future scenario -- a new field-lifter/enricher promoting a
    value onto a brand-new node property ("artifact_content")
...
async def test_process_event_data_fails_closed_when_data_carrier_unregistered(
    ...
    Regression target: this is the exact failure mode the allowlist tripwire
    exists to catch -- a carrier property silently dropping out while the
