...
def test_compose_server_has_healthcheck(compose: dict) -> None:
    ...
    # /status is the unauthenticated Azure Container Apps liveness/health probe
    # and must always stay public — the healthcheck probes it directly.
    assert "curl" in test_str and "localhost:8000/status" in test_str, (
        ...
        "(/status is always unauthenticated — the ACA liveness probe)"
    )
