...
# Queue-backend storage-root attributes that leak an on-disk layout into a
# consumer. ``queues_dir`` is off the QueueManager Protocol -- a consumer that
# reads it (to ``.glob`` or path-join a ``.log``) is reaching around the
# backend-neutral seam and must go through ``session_keys`` instead.
_BANNED_QUEUE_ATTRS = {"queues_dir"}
...
def _violations(path: Path) -> list[str]:
    ...
    for node in ast.walk(tree):
        ...
        # qm.queues_dir reads (leaked queue storage root -- .glob / path-join)
        if isinstance(node, ast.Attribute) and node.attr in _BANNED_QUEUE_ATTRS:
            if (rel, node.attr) in _APPROVED_EXCEPTIONS:
                continue
