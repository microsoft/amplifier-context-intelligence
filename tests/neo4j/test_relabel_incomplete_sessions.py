"""Neo4j integration test -- IncompleteSession backfill.
...
Neo4j container.  Covers six scenarios:
...
(b) node with a linked SessionStartEvent (no terminal label yet) + stale
    ...
    proving the raw selector's mechanics work on this node shape in isolation.
(c) genuine no-start/no-fork node -> RETAINED.
(d) idempotence: a second apply_relabel() call matches zero rows.
(e) the reconciliation gate: seed a linked_but_untyped node and assert
...
are deliberately not a contradiction: (b) proves the underlying raw selector
mechanically heals that shape when invoked directly; (e) proves the
reconciliation safety gate refuses the FULL --apply workflow when such a node
exists anywhere in the graph (the gate exists precisely because the
assumption is unverified on historical data, independent of whether the
mechanics work).
...
#     selector/write function (apply_relabel), proving the raw selector
#     mechanics.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# (e) reconciliation gate: linked_but_untyped node -> --apply REFUSES
# ---------------------------------------------------------------------------
    ...
    def test_linked_but_untyped_node_causes_gated_apply_to_refuse(
        ...
        try:
            with driver.session() as s:
                ...
                assert exit_code == 1, (
                    "run_apply must REFUSE (non-zero exit) when the "
                    "reconciliation diagnostic finds a linked_but_untyped node"
                )
...
# ---------------------------------------------------------------------------
# (g) undo-log is written from a read-only PRE-MUTATION candidate
...
class TestUndoLogCapturesFullPreMutationCandidateSet:
    ...
    shapes that both clear the reconciliation gate: a terminal-labeled node, and a
    ...
    def test_undo_log_exactly_matches_candidates_and_restore_heals_all(
        ...
        try:
            with driver.session() as s:
                ...
                # (so NEITHER trips the linked_but_untyped gate); the
...
class TestUndoLogSourcedFromPreMutationReadNotApplyReturnValue:
    """The undo-log must be sourced from a read-only, pre-mutation
