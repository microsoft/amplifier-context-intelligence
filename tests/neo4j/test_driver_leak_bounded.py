...
(``get_or_create`` -> ``graph_backend.session_store()``) against a live
Neo4j, doing a real write per session so bolt connections are actually
opened, then queries the server's own connection list to prove the open
bolt connections stay bounded by the pool (never scale with the session
count) and are released when the backend is closed.
"""
...
async def _count_python_bolt_connections(probe_driver: Any) -> int:
    ...
    reflects only the backend's shared driver(s).
    """
...
async def test_bolt_connections_stay_bounded_and_release(
    ...
    # writable scratch paths. The registry's non-graph infra (blob/queue
    # paths, write concurrency) still resolves from get_settings(); the graph
    # backend is built separately from the same settings object and bound
    # explicitly, since the registry no longer knows how to build one itself.
    settings = Settings(
        neo4j_url=bolt_url,
        neo4j_user=user,
    ...
    # Both owned pools are established BEFORE the try, and released in its
    ...
    try:
        ...
        async def run_session(i: int) -> None:
            # Exactly the construction path the leak came from: the registry
            # asks the one shared backend for a session store instead of
            # building (or reusing) a driver itself. The write forces a real
            # bolt connection through the pool.
        ...
        # Stop the idle drain workers before tearing the backend down.
        ...
        # Reclaim: closing the shared backend must release every bolt connection.
        ...
        # One shared backend was built for all sessions, not one per session --
        # unbinding it leaves the registry with nothing to hand out a store from.
        ...
        # Reclaim proof: the shared backend's pools are fully released on close.
        assert after_close == 0, (
            f"bolt connections not released after backend close (still {after_close})"
        )
    finally:
        await backend.aclose()
        reg.set_graph_backend(None)
