...
algorithm, only a CLI around the existing functions (DRY, per
``docs/plans/2026-08-13-ws3-implementation-spec.md`` sec 5.1/5.4).
...
WHAT THIS DOES
    ...
    This is NOT a schema-version migration -- WS-3 changes no stored
    node/edge shape, so ``SCHEMA_VERSION`` (``status.py``) stays ``1``.
...
# Self-declared from -> to (spec sec 5.4). This is a server-version /
# structural-rectification step, NOT a schema_version bump -- WS-3 changes
# no stored node/edge shape, so schema_version stays 1 -> 1.
