...
| Server crash-loops on boot with `RuntimeError: Neo4j graph has un-migrated legacy nodes (untagged :Node or duplicates); cold start no longer auto-migrates. Run: context-intelligence-server doctor --fix` | **Un-migrated graph.** Cold start no longer self-heals untagged/duplicate legacy nodes (behavior change — see the [README upgrade note](../README.md#upgrading-cold-start-no-longer-auto-migrates)) | Stop the service, run `context-intelligence-server doctor --fix` to repair the graph, then restart: `sudo systemctl restart context-intelligence-server.service` |
