...
| `api_key` | *(your secret)* | Legacy single bearer token (folds to contributor id `owner`). All endpoints except `/status`, `/version` and static routes require `Authorization: Bearer <value>`. The server verifies it as `sha256(token)`. |
